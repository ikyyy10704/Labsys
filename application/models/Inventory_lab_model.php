...
public function get_calibration_stats($alat_id)
{
    ...
    {
        return $this->db->insert('calibration_history', $data);
    }
    
    /**
     * Get calibration schedule (all alat with jadwal_kalibrasi)
     */
    public function get_calibration_schedule()
    {
        $this->db->select('al.*, 
                          ch.tanggal_kalibrasi as last_calibration,
                          DATEDIFF(al.jadwal_kalibrasi, CURDATE()) as days_until_calibration');
        $this->db->from('alat_laboratorium al');
        $this->db->join('calibration_history ch', 
                       'al.alat_id = ch.alat_id AND ch.tanggal_kalibrasi = al.tanggal_kalibrasi_terakhir', 
                       'left');
