...
<head>
    ...
    <title>Laporan Pemeriksaan - Labsys</title>
...
<!-- Header Section - konsisten dengan user management -->
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-blue-100">Kelola dan analisis data pemeriksaan laboratorium</p>
            </div>
        </div>
...
async function loadExaminationData() {
    try {
        ...
        const response = await fetch(BASE_URL + 'admin/ajax_get_examination_reports?' + new URLSearchParams({
            ...filters,
            page: currentPage,
            per_page: perPage
        }));
...
// View examination detail
async function viewDetail(examinationId) {
    try {
        const response = await fetch(BASE_URL + `admin/ajax_get_examination_detail/${examinationId}`);
        const data = await response.json();
